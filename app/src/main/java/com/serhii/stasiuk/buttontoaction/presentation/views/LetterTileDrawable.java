package com.serhii.stasiuk.buttontoaction.presentation.views;
...
public class LetterTileDrawable extends Drawable {
    ...
    private int mColor;
    private Character mLetterFirst = null;
    private Character mLetterSecond = null;
    ...
    public LetterTileDrawable(final Context context) {
        ...
        if (sColorsLight == null) {
            ...
            sDefaultColor = res.getColor(R.color.grey_blue);
            ...
            DEFAULT_GROUP_AVATAR = getBitmapFromVectorDrawable(context, R.drawable.ic_person_white);
    ...
    public LetterTileDrawable setLetterAndColor(final String displayName, final String identifier) {
        if (displayName == null) mLetterFirst = null;
        else {
            String[] split = displayName.split("\\s+");
            if (split.length > 1) {
                mLetterFirst = Character.toUpperCase(split[0].charAt(0));
                if (split[1].length() > 0)
                    mLetterSecond = Character.toUpperCase(split[1].charAt(0));
    ...
    private void drawLetterTile(final Canvas canvas) {
        ...
        if (mLetterFirst != null) {
            // Draw letter or digit.
            sFirstChar[0] = mLetterFirst;
            if (mLetterSecond != null) sFirstChar[1] = mLetterSecond;
            int size = mLetterSecond != null ? 2 : 1;

            // Scale text by canvas bounds and user selected scaling factor
            sPaint.setTextSize(mScale * sLetterToTileRatio * minDimension);
            //sPaint.setTextSize(sTileLetterFontSize);
            sPaint.getTextBounds(sFirstChar, 0, size, sRect);
            sPaint.setColor(mContactType == ContactType.PERSON ? sTileFontColorDark : sTileFontColorLight);
            // Draw the letter in the canvas, vertically shifted up or down by the user-defined
            // offset
            canvas.drawText(sFirstChar, 0, size, bounds.centerX(),
